...
program main
  ...
  if (any(b1 /= a)) stop 1
  if (any(b2 /= [2, 3, 0])) stop 1
  if (any(b3 /= [0, 0, 1])) stop 1
  if (any(b4 /= 42)) stop 1
  if (any(eoshift(c,shift=1,dim=n1,boundary=33) /= b5)) stop 1
  if (any(eoshift(c,shift=2,dim=1) /= b6)) stop 1
  if (any(eoshift(c,shift=-1,dim=2) /= b7)) stop 1
  if (any(eoshift(c,shift=-1,dim=n2,boundary=[-1,-2,-3]) /= b8)) stop 1
  if (any(eoshift(c,shift=-1) /= b9)) stop 1
  if (any(eoshift(r,shift=1,dim=n3) /= q1)) stop 1
  if (any(b10 /= reshape([ 0, 1, 2, 4, 5, 6, 8, 9, 0],shape(b10)))) stop 1
  if (any(b11 /= reshape([42, 42,  6, 42,  2,  9,  1,  5, 42],shape(b11)))) stop 1
  if (any(b12 /= reshape([ -3,  1,  2, -7, -7,  4,  7,  8,  9],shape(b11)))) stop 1
  ...
  if (any(q2 /= reshape([0.,0.,9.,1600.,841.,0.,0.,0.,2025.,484.,0.,0.,1.,0.,225.,2704.,&
          ...
          0.,0.,0.,0.,0.,0.,0.,2209.,0.],shape(q2)))) stop 1
  ...
  if (any(f2 /= reshape(["AAA","BBB","aaa","bbb","ccc","ddd","CCC","DDD","ggg","hhh","iii","jjj",&
       "EEE","FFF","mmm","nnn","ooo","ppp","GGG","HHH","sss","ttt","uuu","vvv"],shape(f2)))) stop 1

  e2 = e
  if (any (f2 /= eoshift(e2,dim=2,shift=-1,boundary=bnd2))) stop 1
  if (any (f3 /= reshape (["ggg","   ","ccc","jjj","qqq","   ","mmm","   ","iii","ppp",&
       "www","fff","sss","bbb","ooo","vvv","   ","lll","   ","hhh","uuu",&
       "   ","   ","rrr"], shape(f3)))) stop 1
  if (size(empty) /=0) stop 1
  if (any(t /= (0.0_8, 0.0_8))) stop 1
end program main
