...
program associate_18
  ...
  if (any (reshape(am%map, [12]) /= [1,1,1, 1,7,7, 1,8,7, 1,1,1])) stop 1
  deallocate(am)
  ...
  if (any (reshape(pm%map, [12]) /= [1,1,1, 1,7,7, 1,8,7, 1,1,1])) stop 1
  deallocate(pm)

  associate(i => iv(1))
    i = 7
  end associate
  if (any (iv /= [7, 17, 17, 17, 17])) stop 1
  ...
  if (any (reshape(im, [20]) /= [23,23,23,23, 23,9,0,23, &
        23,10,0,23, 23,23,23,23, 23,23,23,23])) stop 1
  ...
  do c = 1, 5
    ...
    if (any (reshape(im, [20]) /= expect)) stop 1
    ! Shift expect
