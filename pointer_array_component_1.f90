...
!
  ...
  y = vtab_r%rvar
  if (any (abs (y - [11.0, 42.0]) > 1.0e-5)) stop 1  ! Check that the component is usable in assignment.
...
contains
  subroutine foobar (vtab, array)
    ...
    real :: array (:)
    if (any (abs (vtab%rvar - array) > 1.0e-5)) stop 1  ! Check passing as a dummy.
    if (abs (vtab%rvar(2) - array(2)) > 1.0e-5) stop 1  ! Check component reference.
  end subroutine
