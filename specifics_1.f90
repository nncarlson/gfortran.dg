...
subroutine test_r2(fn, val1, val2, res)
  ...
  if (diff(fn(val1, val2), res)) stop 1
contains
function diff(a, b)
  real a, b
  logical diff
...
subroutine test_dprod(fn)
  double precision fn
  if (abs (fn (2.0, 3.0) - 6d0) .gt. 0.00001) stop 1
end subroutine

subroutine test_nint(fn,val,res)
  integer fn, res
  real val
  if (res .ne. fn(val)) stop 1
end subroutine
