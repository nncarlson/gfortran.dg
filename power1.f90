...
program power
  ...
  if (abs(rid - 4) > eps_d) stop 1
  if (abs(rie - 4) > eps_e) stop 1
  if (abs(real(cis, s) - 3) > eps_s .or. abs(aimag(cis) - 4) > eps_s) stop 1
  if (abs(real(cid, d) - 3) > eps_d .or. abs(aimag(cid) - 4) > eps_d) stop 1
  if (abs(real(cie, e) - 3) > eps_e .or. abs(aimag(cie) - 4) > eps_e) stop 1

  if (abs(rrs - 4) > eps_s) stop 1
  if (abs(rrd - 4) > eps_d) stop 1
  if (abs(rre - 4) > eps_e) stop 1
  if (abs(real(crs, s) - 3) > eps_s .or. abs(aimag(crs) - 4) > eps_s) stop 1
  if (abs(real(crd, d) - 3) > eps_d .or. abs(aimag(crd) - 4) > eps_d) stop 1
  if (abs(real(cre, e) - 3) > eps_e .or. abs(aimag(cre) - 4) > eps_e) stop 1

  if (abs(rds - 4) > eps_s) stop 1
  if (abs(rdd - 4) > eps_d) stop 1
  if (abs(rde - 4) > eps_e) stop 1
  if (abs(real(cds, s) - 3) > eps_s .or. abs(aimag(cds) - 4) > eps_s) stop 1
  if (abs(real(cdd, d) - 3) > eps_d .or. abs(aimag(cdd) - 4) > eps_d) stop 1
  if (abs(real(cde, e) - 3) > eps_e .or. abs(aimag(cde) - 4) > eps_e) stop 1
