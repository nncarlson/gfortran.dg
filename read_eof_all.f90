...
! Eaxamples from Tobius Burnus 
  ...
  stop 1 !stop 'ERROR: Expected EOF error (1)'
  111 continue

  read(str2,'(i2,/,i2)',end=112) a, b

  read(str2,'(i2,/,i2,/,i2)',end=113) a, b, c
  stop 1 !stop 'ERROR: Expected EOF error (2)'

  112 stop 1 !stop 'ERROR: Unexpected EOF (3)'
  ...
  goto 125
  122 stop 1 !stop 'ERROR: Expected no EOF error (2)'
  125 continue

  read(str2(:),'(i2,/,i2,/,i2)',end=123,pad='no') a, b, c
  stop 1 !stop 'ERROR: Expected EOF error (3)'
  123 continue
  ...
  if (ios /= IOSTAT_END) stop 1 !stop 'ERROR: expected iostat /= 0  (1)'

  read(str2(:)(2:1),'(i2,/,i2)',iostat=ios, pad='no') a, b
  if (ios /= IOSTAT_END) stop 1 !stop 'ERROR: expected iostat /= 0  (2)'
