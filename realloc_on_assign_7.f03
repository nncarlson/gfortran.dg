...
program main
  ...
  call bar
  if (.not. associated (aptr, a)) stop 1 ! reallocated to smaller size - remains associated
  call foobar
  if (associated (aptr, a)) stop 1 ! reallocated to larger size - disassociates
...
!
    ...
    tmp = matmul(a,b)
    if (any (lbound (tmp) .ne. [1,1])) stop 1
    if (any (ubound (tmp) .ne. [10,12])) stop 1
  end subroutine
end program main
