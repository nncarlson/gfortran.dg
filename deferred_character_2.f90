...
! Check that allocation with source = another deferred length is OK
     ...
     if (any (array_copia .ne. array_fijo)) stop 1
...
! Check that allocation with MOLD = a non-deferred length is OK
     ...
     if (size (array_copia, 1) .ne. 4) stop 1
     if (LEN (array_copia, 1) .ne. 2) stop 1
...
!
          ...
          test(1) = 'abc'
          test(2) = 'def'
          if (any (test .ne. ['abc', 'def'])) stop 1

          test = ['aa','bb','cc']
          if (any (test .ne. ['aa', 'bb', 'cc'])) stop 1
