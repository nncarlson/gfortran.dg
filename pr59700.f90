...
20 if (msg /= 'Bad real number in item 4 of list input') stop 1
   rewind(fd)
   msg = 'ok'
   read(fd, *, err=30, iomsg=msg) i1, x2, x1, a
30 if (msg /= 'Bad logical value while reading item 4') stop 1
   rewind(fd)
   read(fd, *, err=31, iomsg=msg) i1, x2, a, x1
31 if (msg /= 'Bad repeat count in item 3 of list input') stop 1
   close(fd)
   open(unit=fd, status='scratch')
   write(fd, '(A)') '(1, 2) (3.4, q)'
   rewind(fd)
...
40 if (msg /= 'Bad complex floating point number for item 2') stop 1
   close(fd)
end program foo
