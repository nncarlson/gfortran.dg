...
!
    ...
    if (ubound (c, 1) .ne. ubound(a, 1)) stop 1
    c=b
...
! for the fix of PR47051. 
    ...
    if (ubound (c, 1) .ne. ubound(a, 1)) stop 1
    d=b
    if (lbound (d, 1) .ne. lbound(b, 1)) stop 1
    if (ubound (d, 1) .ne. ubound(b, 1)) stop 1
    d=a
...
!
    ...
    if (ubound (a, 1) .ne. ubound(b, 1)) stop 1
  end subroutine
  subroutine test3
!
...
!
    ...
    a = f()
    if (any (a .ne. [1, 2, 3, 4])) stop 1
    c = a + 8
    a = f (c)
    if (any ((a - 8) .ne. [1, 2, 3, 4])) stop 1
    deallocate (c)
    a = f (c)
    if (any ((a - 4) .ne. [1, 2, 3, 4])) stop 1
  end subroutine
  function f(b)
...
!
    ...
    if (any (shape (xwrkt) .ne. [ncls, ncls])) stop 1
    xwrkt = invima (xwrkt, ival, ipic, ncls)
    if (nglobal .ne. 1) stop 1
    if (sum(xwrkt) .ne. xwrkt(ival, ival)) stop 1
  end subroutine
  ...
  subroutine test8
    ...
    type(t) :: x
    x%a= [1,2,3]
    if (any (x%a .ne. [1,2,3])) stop 1
    x%a = [4]
    if (any (x%a .ne. [4])) stop 1
  end subroutine
end
