...
contains
  subroutine fini2 (x)
    ...
    !print *, 'fini2:', x%i
    if (any (x%i /= reshape([11, 12, 21, 22, 31, 32], [2,3]))) stop 1
  ...
  subroutine fini3 (x)
    ...
    integer :: i,j,k
    if (.not. elem_call) stop 1
    if (.not. rank3_call) stop 1
    if (cnt2 /= 9) stop 1
    if (cnt /= 1) stop 1
      do i = 1, 2
        do j = 1, 2
          do k = 1, 2
            ...
            if (x(k,j,i)%i /= k+10*j+100*i) stop 1
          end do 
        end do
      end do
    fini_call = fini_call + 1
  ...
  subroutine f2ini2 (x)
    ...
    !print *, 'f2ini2:', x%j
    if (any (x%i /= reshape([11, 12, 21, 22, 31, 32], [2,3]))) stop 1
    if (any (x%j /= 100*reshape([11, 12, 21, 22, 31, 32], [2,3]))) stop 1
    fini_call = fini_call + 1
  end subroutine
  ...
  impure elemental subroutine f2ini_elm (x)
    ...
    if (rank3_call) then
      if (x%i /= exprected(cnt2)) stop 1  
      if (x%j /= 1000*exprected(cnt2)) stop 1  
    else
      if (cnt2 /= x%i .or. cnt2*10 /= x%j) stop 1
    end if
    cnt2 = cnt2 + 1
    fini_call = fini_call + 1
  end subroutine
