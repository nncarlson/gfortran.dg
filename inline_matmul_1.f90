...
program main
  ...
  if (sum(abs(c_alloc-cres))>1e-4) stop 1
  if (any([size(c_alloc,1), size(c_alloc,2)] /= [3,4])) stop 1
  deallocate(c_alloc)
  ...
  bp = b
  cp = matmul(ap, bp)
  if (sum(abs(cp-cres)) > 1e-4) stop 1
  ...
  if (sum(abs(f(3,1:eight:2,:)-cres))>1e-4) stop 1

  afoo%a = a
  bfoo%a = b
  cfoo%a = matmul(afoo%a, bfoo%a)

  if (sum(abs(cfoo%a-cres)) > 1e-4) stop 1
  ...
  block
    ...
    cm = matmul(am, bm)
    if (sum(abs(cm-cres)) > 1e-4) stop 1
