...
subroutine check_int (j)
  ...
  if (sizeof(j) == 4) then
     if (sizeof (j) /= sizeof (i)) stop 1
  else
     if (sizeof (j) /= 2 * sizeof (i)) stop 1
  end if
  ...
  if (sizeof(l) /= 6*sizeof(l(1))) stop 1
...
subroutine check_real (x, y)
  ...
  if (sizeof (y) /= 5*sizeof (x)) stop 1

  if (sizeof (r) /= 8000*4) stop 1
  rp => r(5,2:10,1:5)
  if (sizeof (rp) /= 45*4) stop 1
  rp => r(1:5,1:5,1)
  if (sizeof (d) /= 2*sizeof (rp)) stop 1
  if (sizeof (c(1)) /= 2*sizeof(r(1,1,1))) stop 1
...
subroutine check_derived ()
  ...
  if (sizeof(a) /= sizeof(i)) stop 1
  if (sizeof(oof) /= sizeof(rab)) stop 1
  allocate (v%r(500))
  sizev500 = sizeof (v)
  size_500 = sizeof (v%r)
  deallocate (v%r)
