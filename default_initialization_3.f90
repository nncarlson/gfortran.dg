...
subroutine original
  ...
  if (any (val2 .ne. (/1, 2, 3, 4, 4, 4/))) stop 1
contains
...
subroutine other
  ...
  if ((val1%bar .ne. 42) .or. (val2%bar .ne. 77)) stop 1
  call func (2, val1, val2)
  if ((val1%bar .ne. 42) .or. (val2%bar .ne. 999)) stop 1
...
subroutine dominique ()
  ...
  TYPE(T1) :: D1
  D1=T1(3)
  if (F1(D1) .ne. 7) stop 1
  D1=T1(3)
  if (E1(D1) .ne. 3) stop 1
END
