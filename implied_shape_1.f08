...
PROGRAM main
  ...
  IF (SIZE (arr1) /= 3) stop 1

  IF (LBOUND (arr2, 1) /= 1 .OR. UBOUND (arr2, 1) /= 3) stop 1
  IF (SIZE (arr2) /= 3) stop 1

  IF (ANY (LBOUND (arr3) /= (/ n, 1 /) .OR. UBOUND (arr3) /= (/ n + 1, 2 /))) &
    stop 1
  IF (SIZE (arr3) /= 4) stop 1

  IF (LBOUND (arr4, 1) /= 1 .OR. UBOUND (arr4, 1) /= 2) stop 1
  IF (SIZE (arr4) /= 2) stop 1
END PROGRAM main
