...
!
  ...
  if (any (disp (x) .ne. [0.0,0.0,0.0,0.0])) stop 1
...
contains
  subroutine display(x, lower, upper, t1, t2)
    ...
    select type (x)
      ...
      type is (type2)
        if (present (t2)) then
          if (any (x%i .ne. t2%i)) stop 1
          if (any (x%r .ne. t2%r)) stop 1
        else
          stop 1
        end if
        x%i = 111
  ...
  subroutine bounds (x, lower, upper)
    class(type1), allocatable, dimension (:) :: x
    integer, dimension (:) :: lower, upper
    if (any (lower .ne. lbound (x))) stop 1
    if (any (upper .ne. ubound (x))) stop 1
  end subroutine
