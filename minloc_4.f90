...
program main
  ...
  if (b3 /= 2) stop 1
  if (b4 /= 1) stop 1
  if (any(b5 /= [1, 2])) stop 1
  if (any(b6 /= [0, 0])) stop 1
  if (any(b7 /= [2, 1])) stop 1
  if (any(b8 /= [1, 2])) stop 1
  if (any(b9 /= [0, 0])) stop 1
  d = 1
  if (any(b10 /= minloc(i2,dim=d))) stop 1
  d = 2
  if (any(b11 /= minloc(i2,dim=2))) stop 1
  d = 1
  if (any(b12 /= minloc(i2, dim=d,mask=i2>3))) stop 1
  if (any(b13 /= 0)) stop 1
end program main
