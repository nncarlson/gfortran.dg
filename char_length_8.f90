...
contains
  subroutine test_reshape 
    Z(:)="123"
    if (any (RESHAPE(Z(:)(2:2),(/5,2/)) .ne. "2")) stop 1 
    ctr = ctr + 1
  end subroutine
  ...
  subroutine test_cshift 
    Z(:)="901"
    if (any (CSHIFT(Z(:)(2:2),2) .ne. "0")) stop 1 
    ctr = ctr + 1
  end subroutine
  subroutine test_spread 
    Z(:)="789"
    if (any (SPREAD(Z(:)(2:2),dim=1,ncopies=2) .ne. "8")) stop 1 
    ctr = ctr + 1
  end subroutine
  subroutine test_transpose 
    W(:, :)="abc"
    if (any (TRANSPOSE(W(:,:)(1:2)) .ne. "ab")) stop 1 
    ctr = ctr + 1
  end subroutine
  subroutine test_pack 
    W(:, :)="def"
    if (any (pack(W(:,:)(2:3),mask=.true.) .ne. "ef")) stop 1 
    ctr = ctr + 1
  end subroutine
  subroutine test_unpack 
    ...
    Z(:)="hij"
    mask = .true.
    if (any (unpack(Z(:)(2:2),mask,' ') .ne. "i")) stop 1 
    ctr = ctr + 1
  end subroutine
  subroutine test_pr31197
    ...
    if (any (TRANSPOSE(RESHAPE(T(:)%A(2:2),(/5,2/))) .ne. "y")) stop 1 
    ctr = ctr + 1
  end subroutine
END
