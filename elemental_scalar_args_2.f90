...
!
  ...
  if (any (foo (i, "xy") .ne. ["x", "y"])) stop 1     ! OK - not a function
  if (any (foo (i, chr1) .ne. ["l", "m"])) stop 1     ! ditto
  if (any (foo (i, char (j)) .ne. ["A", "B"])) stop 1 ! This would fail
  if (any (foo (i, chr2) .ne. ["s", "u"])) stop 1     ! OK - not a scalar
  if (any (foo (i, bar2()) .ne. ["e", "g"])) stop 1   ! OK - not a scalar function
contains
