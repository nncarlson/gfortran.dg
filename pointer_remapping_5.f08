...
PROGRAM main
  ...
  IF (ANY (vec /= (/ 1, 3, 5, 7 /))) stop 1
  IF (vec(2) /= 1 .OR. vec(5) /= 7) stop 1
  ...
  IF (ANY (LBOUND (mat) /= (/ 1, 1 /) .OR. UBOUND (mat) /= (/ 3, 2 /))) &
    stop 1
  IF (ANY (mat /= RESHAPE (arr(1:12:2), SHAPE (mat)))) stop 1
  IF (mat(1, 1) /= 1 .OR. mat(1, 2) /= 7) stop 1

  ! Remap with target of rank > 1.
  vec(1:12_1) => basem
  IF (LBOUND (vec, 1) /= 1 .OR. UBOUND (vec, 1) /= 12) stop 1
  IF (ANY (vec /= arr)) stop 1
  IF (vec(1) /= 1 .OR. vec(5) /= 5 .OR. vec(12) /= 12) stop 1
END PROGRAM main
