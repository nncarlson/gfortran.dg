...
contains
  ...
  subroutine check_var_sec(x, factor)
    ...
    do i = 1, 4
      ...
      do j = 1, 5
        j2 = (j-1)*2 + 1
        if (x(j,i) /= (j2 + 100*i2)*factor) stop 1
      end do
    end do
...
program test
  ...
  if (cnt1 + cnt1e + cnt2 + cnt2e + cnt3 + cnt3e /= 0) stop 1

  call fin_test_1(x(::2,::3))
  if (cnt1 /= 5*4) stop 1
  if (cnt1e /= 5*4) stop 1
  cnt1 = 0; cnt1e = 0
  if (cnt2 + cnt2e + cnt3 + cnt3e /= 0) stop 1

  call fin_test_2(y(::2,::3))
  if (cnt2 /= 1) stop 1
  if (cnt2e /= 1) stop 1
  cnt2 = 0; cnt2e = 0
  if (cnt1 + cnt1e + cnt3 + cnt3e /= 0) stop 1

  call fin_test_3(z(::2,::3))
  if (cnt3 /= 1) stop 1
  if (cnt3e /= 1) stop 1
  cnt3 = 0; cnt3e = 0
  if (cnt1 + cnt1e + cnt2 + cnt2e /= 0) stop 1
    ...
    do i = 1, 10
      do j = 1, 10
        if (mod (j-1, 2) == 0 .and. mod (i-1, 3) == 0) then
          ...
          if (x(j,i) /= (j + 100*i)*factor) stop 1
        end if
      end do
    end do
