...
subroutine pr54990
  ...
  select type (q => a%cBh(2)) ! Similarly, reference 2nd element to test offset
    type is (ncBhStd)
      stop 1
    type is (ncBhStde)
      if (q%i .ne. 198) stop 1 ! This tests that the component really gets the
...
subroutine pr54992  ! This test remains as the original.
  ...
  b%cBh(1)%hostNode => b
! #1 this worked
  if (loc(b) .ne. loc(b%cBh(1)%hostNode)) stop 1
  call Node_C_Bh_Move(b)
! #2 this worked
  if (loc(b) .ne. loc(b%cBh(1)%hostNode)) stop 1
  if (loc(b) .ne. loc(b%cBh(2)%hostNode)) stop 1
! #3 this did not
  bh => bhGet(b,instance=1)
  if (loc (b) .ne. loc(bh%hostNode)) stop 1
