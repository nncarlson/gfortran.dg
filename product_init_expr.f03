...
!
  ...
  IF (.NOT. ALL ([r_equal_prod, r_empty_prod])) stop 1
...
CONTAINS
  SUBROUTINE ilib (array, result)
    ...
    IF (PRODUCT(array) /= result) stop 1
  END SUBROUTINE
  ...
  SUBROUTINE ilib_with_dim (array, dim, result)
    ...
    IF (ANY (PRODUCT (array, dim=dim) /= result)) stop 1
  END SUBROUTINE

  SUBROUTINE rlib (array, result)
    REAL, DIMENSION(:,:), INTENT(in) :: array
    REAL, INTENT(in) :: result
    IF (ABS(PRODUCT(array) - result) > 2e-6) stop 1
  END SUBROUTINE
  ...
  SUBROUTINE rlib_with_dim (array, dim, result)
    ...
    IF (ANY (ABS(PRODUCT (array, dim=dim) - result) > 2e-6)) stop 1
  END SUBROUTINE
END
