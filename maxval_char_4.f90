...
program main
  ...
  if (ret(1) /= zero) stop 1
  
  r1 = 4_'qq'
  r1 = maxval(a, dim=1, mask=a>4_"000200");
  if (any(r1 /= zero .neqv. maxval(v,dim=1, mask=v>200) > 0)) stop 1
  if (any(maxval(a, dim=1, mask=a>4_"000200") /= zero .neqv. maxval(v,dim=1, mask=v>200) > 0)) stop 1

  r1 = 4_'rr'
  r1 = maxval(a, dim=2, mask=a>4_"000200");
  if (any(r1 /= zero .neqv. maxval(v,dim=2, mask=v>200) > 0)) stop 1
  if (any(maxval(a, dim=2, mask=a>4_"000200") /= zero .neqv. maxval(v,dim=2, mask=v>200) > 0)) stop 1
