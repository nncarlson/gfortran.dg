...
!print *, s, v
if (s /= 4*2) stop 1
if (any (v /= [4*2, 4*2])) stop 1

call sub1 (s, x%p, .true.)
call sub1 (v, x%p, .true.)
!print *, s, v
if (s /= 5*2) stop 1
if (any (v /= [5*2, 5*2])) stop 1
...
!print *, v
if (any (v /= [84*2, 82*2])) stop 1

call sub1 (v, x%p2, .true.)
!print *, v
if (any (v /= [35*2, 58*2])) stop 1
...
!print *, s, v
if (s /= 6*2) stop 1
if (any (v /= [6*2, 6*2])) stop 1

call sub_t (s, cp, .true.)
call sub_t (v, cp, .true.)
!print *, s, v
if (s /= 7*2) stop 1
if (any (v /= [7*2, 7*2])) stop 1
...
select type (caa)
type is (t)
  if (any (caa(:)%a /= [66, 666])) stop 1
end select

select type (cpa)
type is (t)
  if (any (cpa(:)%a /= [77, 777])) stop 1
end select

call sub_t (v, taa, .true.)
!print *, v
if (any (v /= [44*2, 444*2])) stop 1

call sub_t (v, tpa, .true.)
!print *, v
if (any (v /= [55*2, 555*2])) stop 1


call sub_t (v, caa, .true.)
!print *, v
if (any (v /= [66*2, 666*2])) stop 1

call sub_t (v, cpa, .true.)
!print *, v
if (any (v /= [77*2, 777*2])) stop 1
